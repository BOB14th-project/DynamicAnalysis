...
1. **정적 분석 단계** (`main.py`)
    - 대상 바이너리 파일 타입 식별 (ELF, PE, Mach-O)
    - `python-magic` 라이브러리로 파일 형식 검증
    - 동적 라이브러리 의존성 탐색 (`ldd` 명령어 활용)

2. **프로세스 생성 및 어태치**
    - Frida로 대상 바이너리를 **새 프로세스로 spawn** 
    - 생성된 프로세스에 **일시정지 상태로** attach
    - PID 획득 및 세션 연결

3. **JavaScript Agent 주입**
    - `c_cpp_agent.js` 파일을 대상 프로세스 메모리 공간에 주입
    - Agent가 **런타임에 동적으로** 암호화 함수들을 탐지하고 후킹
    - 함수 호출 전후의 메모리 상태 캡처

4. **실시간 모니터링**
    - 프로세스 실행 재개 (`resume`)
    - 설정된 시간(기본 10초) 동안 모든 함수 호출 추적
    - 실시간으로 데이터 수집 및 Python으로 전송

5. **결과 수집 및 보고서 생성**
    - JSON 형태로 모든 캡처 데이터 저장
    - 후처리 스크립트들로 CSV, TXT 형태 변환
